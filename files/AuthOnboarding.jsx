...
import {
  AMBER, AMBER_BG, AMBER_LIGHT,
  GREEN, GREEN_BG,
  INK, INK_SOFT,
  GRAY_TEXT, GRAY_SUB, GRAY_MUTED, GRAY_BG, GRAY_BORDER,
  PAGE_BG, CARD_BG, FONT,
  BORDER, BORDER_MED,
...
const inputStyle = {
  ...
  border: `1px solid rgba(0,0,0,0.12)`, borderRadius: RADIUS_MD,
  outline: "none", boxSizing: "border-box", color: INK,
  background: CARD_BG, fontFamily: FONT,
  transition: "border-color 0.15s",
};

const labelStyle = {
  fontSize: 12, fontWeight: 500, color: INK, display: "block", marginBottom: 6,
};
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <input
        ...
        style={{ ...inputStyle, borderColor: focused ? AMBER : "rgba(0,0,0,0.12)" }}
      />
    </div>
  );
}
  ...
  return (
    <button
      ...
      style={{
        ...
        background: disabled ? GRAY_BG : AMBER,
        color: disabled ? GRAY_MUTED : "white",
        border: "none", borderRadius: RADIUS_LG, cursor: disabled ? "not-allowed" : "pointer",
...
function Divider({ label = "or" }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "18px 0" }}>
      ...
      <span style={{ fontSize: 12, color: GRAY_MUTED }}>{label}</span>
      <div style={{ flex: 1, height: 1, background: "rgba(0,0,0,0.08)" }} />
    </div>
  );
}
...
function RolePicker({ onSelect }) {
  ...
  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontSize: 26, fontWeight: 700, letterSpacing: -0.5, color: INK, marginBottom: 6 }}>
        Welcome to Line<span style={{ color: AMBER }}>Up</span>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {[{role:"barber",icon:"ti-scissors",title:"I'm a Barber",sub:"Set up your storefront, manage schedule, fill your book",accent:AMBER,accentBg:AMBER_BG},{role:"customer",icon:"ti-user",title:"I'm a Customer",sub:"Find barbers, book appointments, join the ASAP queue",accent:GREEN,accentBg:GREEN_BG}].map(item => (
          <div key={item.role} onClick={() => onSelect(item.role)} onMouseEnter={() => setHovered(item.role)} onMouseLeave={() => setHovered(null)}
            style={{ background: hovered === item.role ? item.accentBg : CARD_BG, border: `1.5px solid ${hovered === item.role ? item.accent : "rgba(0,0,0,0.1)"}`, borderRadius: RADIUS_LG, padding: "20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 16, transition: "all 0.15s", textAlign: "left" }}>
            <div style={{ width: 48, height: 48, borderRadius: RADIUS_MD, flexShrink: 0, background: hovered === item.role ? item.accent : item.accentBg, display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.15s" }}>
              <i className={`ti ${item.icon}`} style={{ fontSize: 22, color: hovered === item.role ? "white" : item.accent }} />
            </div>
            <div>
      ...
      <div style={{ marginTop: 28, fontSize: 13, color: GRAY_TEXT }}>Already have an account?{" "}<span style={{ color: AMBER, cursor: "pointer", fontWeight: 500 }}>Log in</span></div>
    </div>
  );
}
    ...
    <div style={{ fontFamily: FONT, background: PAGE_BG, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "32px 16px" }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ background: CARD_BG, border: BORDER, borderRadius: RADIUS_XL, padding: "32px 28px", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
          {screen === "pick" && <RolePicker onSelect={r => { setRole(r); setScreen("signup"); }} />}
          {screen === "signup" && (
            <div>
          ...
          {screen === "login" && (
            <div>
              ...
              <PrimaryBtn onClick={() => setScreen("done")}>Log in</PrimaryBtn>
              <div style={{ textAlign: "center", marginTop: 16, fontSize: 13, color: GRAY_TEXT }}><span onClick={() => setScreen("signup")} style={{ color: AMBER, cursor: "pointer" }}>← Sign up instead</span></div>
            </div>
          )}
          {screen === "onboard" && (
            <div>
          ...
          {screen === "done" && (
            <div style={{ textAlign: "center" }}>
              ...
              <PrimaryBtn onClick={() => { setScreen("pick"); setRole(null); setOnboardStep(1); }}>{role === "barber" ? "Go to dashboard →" : "Explore barbers →"}</PrimaryBtn>
            </div>
          )}
        </div>
        {(screen === "signup" || screen === "login") && (
          <div style={{ textAlign: "center", marginTop: 16 }}>
            <span onClick={() => setScreen("pick")} style={{ fontSize: 12, color: GRAY_TEXT, cursor: "pointer" }}>← Change account type</span>
          </div>
        )}
      </div>
    </div>
